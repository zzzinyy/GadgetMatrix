...
export async function trackEvent(productId: string, eventType: EventType) {
  try {
    ...
    console.error("No se pudo registrar la interacción del producto", error);
  }
}
...
export function eventsQuery(days: number) {
  return queryOptions({
    ...
    queryFn: async ({ signal }): Promise<RawEvent[]> => {
      // Los contadores y la gráfica abarcan los mismos días naturales en UTC.
      ...
      for (;;) {
        const { data, error } = await supabase
          ...
          .range(events.length, events.length + 999)
          .abortSignal(signal);
        if (error) throw error;
        if (!data?.length) return events;
        // No asumir que el servidor permite 1000 filas por respuesta.
...
export function buildDailySeries(events: RawEvent[], days: number) {
  const buckets = new Map<
    string,
    { day: string; view: number; card_click: number; affiliate_click: number }
  >();
...
export function buildProductTotals(events: RawEvent[], names: Map<string, string>) {
  ...
  for (const event of events) {
    ...
    if (
      ...
      continue;
    const current = totals.get(event.product_id) ?? {
      id: event.product_id,
