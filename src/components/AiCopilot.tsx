...
export function AiCopilot() {
  ...
  const mutation = useMutation({
    mutationFn: (next: AiMessage[]) => callAgent({ data: { messages: next } }),
    onSuccess: (result) => {
      setMessages((prev) => [...prev, { role: "assistant", content: result.reply }]);
      if (result.actions.length > 0) {
        toast.success(`Cambios aplicados: ${result.actions.length}`);
  ...
  function send(text: string) {
    ...
    const next: AiMessage[] = [...messages, { role: "user", content: clean }];
    setMessages(next);
    setInput("");
  ...
  return (
    <section className="mt-8 rounded-xl border border-border bg-card p-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Pídele que cree productos, fichas técnicas, artículos o listas Top: los aplica en la base
            de datos.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 max-h-[420px] space-y-4 overflow-y-auto rounded-lg border border-border bg-surface p-4">
        ...
        {mutation.isPending ? (
          <p className="flex items-center gap-2 text-sm text-muted-foreground">
            <Loader2 className="size-4 animate-spin" /> Trabajando…
          </p>
        ) : null}
      </div>
      ...
      <div className="mt-4 flex gap-2">
        ...
        />
        <Button onClick={() => send(input)} disabled={mutation.isPending} className="self-end">
